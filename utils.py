...
# copy-on-write will be default in new versions of pandas
# so, they recommend doing this in the meantime
...
def threshold(pinnacle_dawg, pinnacle_fav):
    ...
    """
    underdog_prob, fav_prob, no_vig_underdog, no_vig_fav = remove_vig(
        pinnacle_dawg, pinnacle_fav
    )
...
def remove_vig(underdog, favorite):
    ...
    vig = 100 * (
        1
        - (
            (underdog_decimal * favorite_decimal)
            / (underdog_decimal + favorite_decimal)
        )
    )
    # random vig formula
...
def manual(
    dawg, fav, pinnacle_dawg, pinnacle_fav
):  # manually input odds & see expected value and kelly bet size
    underdog_prob, fav_prob, no_vig_underdog, no_vig_fav = remove_vig(
        pinnacle_dawg, pinnacle_fav
    )
...
def sportsbook_data(region, sport, bet_type, API_KEY=API_KEY):
    ...
    MARKETS = (
        bet_type  # h2h | spreads | totals. Multiple can be specified if comma delimited
    )
        ...
        for entry in current_game.bookmakers:
            for book in entry:
                ...
                try:  # if some games not found and/or listed on pinnacle
                    ...
                    under_expectation, under_kelly, fav_expectation, fav_kelly = manual(
                        dog_odds, fav_odds, int(pinnacle_dog.iloc[0]), int(pinnacle_favorite.iloc[0])
                    )
                    # update lists
                    ## favorite
...
def mine_helper(relevant, current_pinnacle, side_data, side):
    ...
    best["pinnacle_fav_odds"] = current_pinnacle.iloc[0, current_pinnacle.columns.get_loc('favorite_odds')]
    best["pinnacle_dog_odds"] = current_pinnacle.iloc[0, current_pinnacle.columns.get_loc('underdog_odds')]
    best["pinnacle_spread"] = abs(
        abs(best.pinnacle_fav_odds) - abs(best.pinnacle_dog_odds)
    )
...
def mine_data(american_dict, pinnacle_dict):
    ...
    for game in american_dict:
        ...
        try:  # if cant find game in pinnacle, ob
